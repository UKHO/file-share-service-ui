...
describe('EssAddSingleEncsComponent', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(service, 'getValidEncs').mockReturnValue([]);
  ...
  it('should set aioCell to undefined when aioCell config is not available', () => {
    ...
    expect(component.aioCell).toBe('');
  });

  it('should set showAio true when component is created', () => {
    expect(component.showAio).toBeTruthy();
  });

  it('checkAioVisibility should set showAio to true', () => {
    component.checkAioVisibility();
    expect(component.showAio).toBeTruthy();
  });

  it('checkAioVisibility should set showAio to false when aioCell is empty', () => {
    component.aioCell = '';
    component.checkAioVisibility();
    expect(component.showAio).toBeFalsy();
  });

  it('checkAioVisibility should set showAio to false when validEnc includes aioCell', () => {
    component.validEnc = ['GB800002'];
    component.checkAioVisibility();
    expect(component.showAio).toBeFalsy();
  });
  ...
  it('should return sales catalogue Response when user is in encList screen', fakeAsync(() => {
    ...
    component.txtSingleEnc = 'AU210470';
    component.renderedFrom = 'encList';
    const checkAioVisibilityMock = jest.spyOn(component, 'checkAioVisibility');
    service.setValidENCs(component.validEnc);
    jest.spyOn(scsProductInformationApiService, 'scsProductIdentifiersResponse').mockReturnValue(of(scsProductUpdatesByIdentifiersMockData));
    component.productUpdatesByIdentifiersResponse(component.validEnc, component.renderedFrom)
    tick();
    expect(component.displayLoader).toEqual(false);
    expect(5).toEqual(scsProductUpdatesByIdentifiersMockData.productCounts.returnedProductCount);
    expect(component.txtSingleEnc).toBe('');
    expect(component.isAioChecked).toBeFalsy();
    expect(checkAioVisibilityMock).toHaveBeenCalled();
  ...
  test('onClick should toggle isAioChecked', () => {
    ...
    component.onClick();
    expect(component.isAioChecked).toBeTruthy();
    expect(component.txtSingleEnc).toBe(component.aioCell);
    component.onClick();
    expect(component.isAioChecked).toBeFalsy();
    expect(component.txtSingleEnc).toBe('');
  });
